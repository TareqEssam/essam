...
window.isDecision104Question = function(query) {
    ...
    const patterns = [
        ...
        // ✅ [إضافة] أنماط الإدراج والاستفادة
        /\b(وارد|مدرج|مشمول|يستفيد|يندرج|ينضوي|يخضع)\b/,
        /\b(ضمن|داخل|في)\s*(القرار|قرار)\b/,
        /هل\s*(هو|هي|ده|دي)?\s*(له|لها)?\s*(مزايا|حوافز|اعفاء|إعفاء)/,
        /هل\s*(هو|هي|ده|دي)?\s*(مشمول|مدرج|وارد|يستفيد|يستحق)/,
        /\b(يستحق|يحق\s*له|يحق\s*لها|يستحقها)\b/,
        /\b(اعفاء|إعفاء)\s*(ضريبي|كلي|جزئي)\b/,
        /\b(له|لها)\s*(حوافز|مزايا|اعفاء|إعفاء)\b/
    ];
