...
function isQueryRelatedToContext(query, context) {
    ...
    if (context.type === 'industrial') {
        ...
        // ── 🆕 [حارس السؤال العام] ──────────────────────────────────────────
        // السؤال من نوع "ما هي جهات الولاية للمناطق الصناعية؟" هو سؤال عام جديد
        // يسأل عن كل المناطق دفعةً واحدة، وليس عن منطقة بعينها في السياق.
        //
        // معيار التمييز (3 شروط مجتمعة = سؤال عام جديد → غير مرتبط):
        //   1. يحتوي على جمع "مناطق" أو "الصناعية" + جمع
        //   2. لا يذكر اسم المنطقة الحالية في السياق
        //   3. طوله > 10 أحرف (ليس اختصاراً قصيراً كـ "جهه")
        // ─────────────────────────────────────────────────────────────────────
        const currentAreaName = normalizeArabic(context.data?.name || '');
        const isAskingAboutAllAreas = (
            /مناطق|الصناعيه.*كل|كل.*الصناعيه|جميع.*منطق|منطق.*جميع/.test(q) ||
            (/للمناطق|المناطق الصناعيه|المناطق الصناعية/.test(q))
        );
        const mentionsCurrentArea = currentAreaName.length > 3 &&
            currentAreaName.split(/\s+/).filter(w => w.length > 3)
                           .some(w => q.includes(w));

        if (isAskingAboutAllAreas && !mentionsCurrentArea && q.length > 10) {
            console.log("🆕 [حارس عام] سؤال عام عن كل المناطق وليس عن المنطقة الحالية - سؤال جديد");
            return false;
        }
        // ─────────────────────────────────────────────────────────────────────
